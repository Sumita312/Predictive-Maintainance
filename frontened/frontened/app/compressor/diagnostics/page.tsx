...
const compressorData = [
  { id: "CMP-001", rpm: 2950, power: 15.4, pressure: 7.5, airflow: 320, temp: 85, risk: 88, status: "FAULT" },
  { id: "CMP-002", rpm: 2800, power: 14.2, pressure: 6.8, airflow: 310, temp: 82, risk: 62, status: "DEGRADED" },
  { id: "CMP-003", rpm: 3000, power: 16.1, pressure: 7.9, airflow: 330, temp: 88, risk: 71, status: "FAULT" },
  { id: "CMP-004", rpm: 2750, power: 13.9, pressure: 6.5, airflow: 300, temp: 78, risk: 14, status: "NORMAL" },
  { id: "CMP-005", rpm: 3100, power: 17.0, pressure: 8.1, airflow: 340, temp: 90, risk: 8,  status: "NORMAL" },
  { id: "CMP-006", rpm: 2890, power: 15.0, pressure: 7.2, airflow: 315, temp: 84, risk: 55, status: "DEGRADED" },
  { id: "CMP-007", rpm: 2700, power: 13.5, pressure: 6.3, airflow: 295, temp: 77, risk: 11, status: "NORMAL" },
  { id: "CMP-008", rpm: 3050, power: 16.5, pressure: 8.0, airflow: 335, temp: 89, risk: 6,  status: "NORMAL" },
];
...
function StatCard({ label, value, sub, color }: { label: string; value: number; sub: string; color?: string }) {
  return (
    <div style={{
      ...
      padding: "20px 22px", border: "1px solid rgba(255,255,255,0.07)",
      position: "relative", overflow: "hidden",
    }}>
      {color && (
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${color}, ${color}33)`, borderRadius: "16px 16px 0 0" }} />
      )}
...
function ActionCard({ icon, title, desc, cta, streak, accentColor, accentBg, tags, onClick }: ActionCardProps) {
  ...
  return (
    <button
      ...
      style={{
        flex: 1, background: hovered ? `rgba(${accentBg}, 0.05)` : "rgba(255,255,255,0.025)",
    ...
    >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
          {tags.map(tag => (
            <span key={tag} style={{
              ...
              padding: "4px 10px", borderRadius: 6, background: `rgba(${accentBg}, 0.08)`,
      ...
      <div style={{
        ...
        color: hovered ? accentColor : `rgba(${accentBg}, 0.6)`, transition: "all 0.3s"
      }}>
        <span>{cta}</span>
...
export default function CompressorDiagnostics({ onBack }: Props) {
  const [view, setView] = useState<"dashboard" | "fleet">("dashboard");
  const [filter, setFilter] = useState("All");
  const router = useRouter();

  const normal   = compressorData.filter(m => m.status === "NORMAL").length;
  const degraded = compressorData.filter(m => m.status === "DEGRADED").length;
  const fault    = compressorData.filter(m => m.status === "FAULT").length;

  const filtered = compressorData.filter(m =>
    filter === "All" ? true :
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#04091a", fontFamily: "'DM Sans', sans-serif", color: "#e2e8f0", position: "relative", overflow: "hidden" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .diag-back-btn {
          ...
          color: #5a6a88; border-radius: 8px; padding: 6px 14px; cursor: pointer;
        ...
        .filter-tab {
          padding: 6px 18px; border-radius: 8px; font-size: 12px;
          ...
          background: transparent; color: #5a6a88;
        }
      ...
      <div style={{
        ...
        background: `
          radial-gradient(ellipse 70% 55% at 15% 25%, rgba(14,165,160,0.07) 0%, transparent 60%),
          radial-gradient(ellipse 60% 50% at 85% 75%, rgba(244,121,32,0.05) 0%, transparent 60%),
          radial-gradient(ellipse 55% 45% at 75% 15%, rgba(0,48,135,0.04) 0%, transparent 55%),
      ...
      <div style={{ position: "relative", maxWidth: 900, margin: "0 auto", padding: "48px 24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 3 }}>
                <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "1.4rem", fontWeight: 700, color: "#f0f4ff", letterSpacing: "-0.3px" }}>Compressor Diagnostics</span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 40 }}>
          <StatCard label="Total Compressors" value={compressorData.length} sub="Across 2 facilities" />
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              <ActionCard
                icon={
                  <svg viewBox="0 0 64 64" width={28} height={28} fill="none" stroke="currentColor" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <circle cx="32" cy="32" r="5" fill="currentColor" opacity="0.4" stroke="none"/>
                  </svg>
                }
                title="New Prediction"
                desc="Enter sensor readings for a single compressor — RPM, airflow, pressure and temperature — and get an instant NORMAL / DEGRADED / FAULT classification."
                cta="Run single compressor analysis"
                streak="#0ea5a0"
                accentColor="#0ea5a0"
                accentBg="14,165,160"
                tags={["14 Sensors", "State Detection", "Fault Classification"]}
              ...
              <ActionCard
                ...
                desc="View all 8 compressors ranked by failure probability. See sensor trend data, fault states, and recommended maintenance windows."
                cta="View fleet risk overview"
                streak="#F47920"
                accentColor="#F47920"
                accentBg="244,121,32"
                tags={["8 Compressors", "Risk Ranking", "Maintenance Windows"]}
                onClick={() => setView("fleet")}
              />
            </div>
            <p style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: "0.68rem", fontWeight: 500, color: "#2a3450", letterSpacing: "1px", textAlign: "center", marginTop: 40 }}>
              Models trained on real industrial sensor datasets &nbsp;·&nbsp; Multi-class classification
            </p>
          </>
        )}
        ...
        {view === "fleet" && (
          <>
                  ...
                  {filtered.map((c) => (
                    <tr key={c.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      <td style={{ padding: "14px 18px" }}>
                        <span style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 14, fontWeight: 700, color: "#0ea5a0", letterSpacing: "0.04em" }}>{c.id}</span>
                      </td>
                      <td style={{ padding: "14px 18px", fontFamily: "'Rajdhani', sans-serif", fontSize: 13, fontWeight: 600, color: "#8899bb" }}>{c.rpm}</td>
                      ...
                      <td style={{ padding: "14px 18px" }}><RiskBar value={c.risk} status={c.status as Status} /></td>
                      <td style={{ padding: "14px 18px" }}><StatusBadge status={c.status as Status} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 11, fontWeight: 600, color: "#2a3450", letterSpacing: "1px", marginTop: 14, textAlign: "right" }}>
              SHOWING {filtered.length} OF {compressorData.length} COMPRESSORS
            </div>
          </>
        )}
      </div>
